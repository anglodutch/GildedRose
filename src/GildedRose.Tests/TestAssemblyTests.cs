...
namespace GildedRose.Tests
{
	...
	{
		protected void TestItem(IUpdateItem updateItem, int expectedSellIn, int expectedQuality)
		{
			updateItem.Update();
	...
	public class ConcertStateTestBase
	{
		protected void TestUpdate(IConcertState state, Item item, int expectedQuality)
		{
			state.Update(item);
		...
		protected void TestNextState(IConcertState state, Item item,  Type expecteNextState)
		{
			var nextState = state.Update(item);

			Assert.IsType(expecteNextState, nextState);
		}

	}

	public class ConcertNormalStateTest : ConcertStateTestBase
	{
		[Theory]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 11, 49, 50)]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 11, 50, 50)]
		public void TestNotAboveFifty(string itemName, int sellInStart, int qualityStart, int expectedQuality)
		{
			TestUpdate(new ConcertStateNormal(), new Item { Name = itemName, SellIn = sellInStart, Quality = qualityStart }, expectedQuality);
		}

		[Theory]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 11, 25, typeof(ConcertStateSoon))]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 12, 25, typeof(ConcertStateNormal))]
		public void TestStateChange(string itemName, int sellInStart, int qualityStart, Type expectednextState)
		{
			TestNextState(new ConcertStateNormal(), new Item { Name = itemName, SellIn = sellInStart, Quality = qualityStart }, expectednextState);
		}
	}

	public class ConcertSoonStateTest : ConcertStateTestBase
	{
		[Theory]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 10, 49, 50)]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 10, 48, 50)]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 10, 50, 50)]
		public void TestNotAboveFifty(string itemName, int sellInStart, int qualityStart, int expectedQuality)
		{
			TestUpdate(new ConcertStateSoon(), new Item { Name = itemName, SellIn = sellInStart, Quality = qualityStart }, expectedQuality);
		}

		[Theory]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 6, 25, typeof(ConcertStateReallySoon))]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 7, 25, typeof(ConcertStateSoon))]
		public void TestStateChange(string itemName, int sellInStart, int qualityStart, Type expectednextState)
		{
			TestNextState(new ConcertStateSoon(), new Item { Name = itemName, SellIn =  sellInStart, Quality = qualityStart }, expectednextState);
		}
	}

	public class ConcertReallySoonStateTest : ConcertStateTestBase
	{
		[Theory]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 5, 50, 50)]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 5, 49, 50)]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 5, 48, 50)]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 5, 47, 50)]
		public void TestNotAboveFifty(string itemName, int sellInStart, int qualityStart, int expectedQuality)
		{
			TestUpdate(new ConcertStateReallySoon(), new Item { Name = itemName, SellIn = sellInStart, Quality = qualityStart }, expectedQuality);
		}

		[Theory]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 0, 25, typeof(ConcertStatePassed))]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 1, 25, typeof(ConcertStateReallySoon))]
		public void TestStateChange(string itemName, int sellInStart, int qualityStart, Type expectednextState)
		{
			TestNextState(new ConcertStateReallySoon(), new Item { Name = itemName, SellIn = sellInStart, Quality = qualityStart }, expectednextState);
		}
	}

	public class ConcertPassedStateTest : ConcertStateTestBase
	{
		[Theory]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 0, 50, 0)]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 0, 25, 0)]
		[InlineData("Backstage passes to a TAFKAL80ETC concert", 0, 10, 0)]
		public void TestQuaityAlways0(string itemName, int sellInStart, int qualityStart, int expectedQuality)
		{
			TestUpdate(new ConcertStatePassed(), new Item { Name = itemName, SellIn = sellInStart, Quality = qualityStart }, expectedQuality);
		}
		...
		public void TestStateChange()
		{
			for (int i = 0; i > -50; i--)
			{
				TestNextState(new ConcertStatePassed(), new Item { Name = "Backstage passes to a TAFKAL80ETC concert", SellIn = i, Quality = 25 }, typeof(ConcertStatePassed));
			}
